...
public class Box implements Iterable<Ball> {
    ...
    public boolean add(Ball b) {
        if(contains(b))
        	return false;
        else if(b.getVolume() + getVolume() > box_volume)
        	return false;
        else
        {
        	ballContainer.add(b);
    ...
    public Iterator<Ball> getBallsFromSmallest() {
    	TreeSet<Ball> ball = new TreeSet<Ball>(new ballComparator());
        Iterator<Ball> ball_it = iterator();
        while (ball_it.hasNext())
        	ball.add(ball_it.next());
        return ball.iterator();
    }

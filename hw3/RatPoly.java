...
public final class RatPoly {
	...
	private static void scaleCoeff(RatNum[] arr, RatNum scalar) {
    	if(!(scalar.isPositive() && scalar.isNegative()))
    		arr = new RatNum[0];
    	else
    	{
    ...
    public RatPoly negate() {
        ...
        {
        	RatNum [] temp = new RatNum[coeffs.length];
        	for(int i = 0; i < coeffs.length; i++)
        		temp[i] = coeffs[i].negate();
    ...
    public RatPoly add(RatPoly p) {
    	...
    	{
    		RatNum [] r = new RatNum[p.coeffs.length];
    	...
    	else
    	{
    		...
    		else
    		{
    			...
    			if(r[r.length - 1].equals(new RatNum(0)))
    				return ZERO;
        		return new RatPoly(r);
    		}
    	}
    ...
    public RatPoly mul(RatPoly p) {
    	...
    	{
    		RatNum [] r = new RatNum[coeffs.length+ p.coeffs.length - 1];
    		for(int i =0; i < r.length; i++)
    			r[i] = new RatNum(0);
     		for(int i = 0; i < this.coeffs.length; i++)
    		{
    ...
    public RatPoly div(RatPoly p)
    {
    	...
    	else
    	{
    		...
    		while(r.length >= p.coeffs.length && r.length > 0 && k >= 0)
    		{
    			...
    			q[k] = t;
    			for(int i = r.length - j + 1 - p.coeffs.length; i < r.length - j + 1; i++)
        			r[i] = r[i].sub(t.mul(p.coeffs[i + j + p.coeffs.length - 1 - r.length]));
    			k = k - 1;
    			j = j + 1;
    		}
    ...
    public double eval(double d) {
        ...
        else
        {
            ...
            for (int i = this.coeffs.length - 1; i >= 0; i--)
                result = this.coeffs[i].doubleValue() + (d * result);
            return result;
        }
    }  
    ...
    public RatPoly differentiate() {
        ...
        else
        {
			...
			RatNum [] r = new RatNum[this.coeffs.length - 1];
        	for (int i = 0; i < r.length; i++)
        		r[i] = this.coeffs[i + 1].mul(new RatNum(i + 1));
        	return new RatPoly(r);
        }
    }
    ...
    public RatPoly antiDifferentiate(RatNum integrationConstant) {
        ...
        else
        {
        	...
        	{
        		r[i] = this.coeffs[i - 1].div(new RatNum(i));
        	}
        	r[0] = integrationConstant;
        	if(r[r.length - 1].equals(new RatNum(0)))
        		return ZERO;
        	checkRep();
        	return new RatPoly(r);
    ...
    public double integrate(double lowerBound, double upperBound) {
    	...
    	{
    		RatPoly aD = antiDifferentiate(new RatNum(1));

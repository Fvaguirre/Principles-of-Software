...
{	
	...
	public static BooleanExp parse(String str)
	{
		...
		else if (token.equals("NOT "))
		{
			...
			if ((left == null) || (right == null))
				return null;
			return new BooleanExp(4,left,right,null,null);			
		}
	...
	private String print(boolean preorder, BooleanExp exp)
	{
		...
		else// print Inorder, getting rid of redundant parentheses
		{
			...
			{
				case BooleanExp.AND: 
					if (exp.getLeft().getExpressionCode() > BooleanExp.AND)// if left operand is an expression of equal or lower precedence, parens are needed
					{
						result.append("(");
						result.append(print(preorder,exp.getLeft()));
						result.append(")");
					}
					else// otherwise, i.e., of higher precedence, no parens needed
						result.append(print(preorder,exp.getLeft()));
					result.append(" AND ");
	...
	{		
		if (exp.getExpressionCode() == BooleanExp.AND)
				return evaluate(context, exp.getLeft()) && evaluate(context, exp.getRight());
